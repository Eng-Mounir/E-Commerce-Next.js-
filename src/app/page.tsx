...
export default function Home() {
  ...
  return (
    <>
      <style>{`
        @import url('https://fonts.googleapis.com/css2?family=Cormorant+Garamond:ital,wght@0,500;0,600;0,700;1,500;1,600;1,700&family=DM+Sans:ital,wght@0,300;0,400;0,500;0,600;1,300&display=swap');
        ...
        * { box-sizing: border-box; }
        ...
        @keyframes pulse2   { 0%,100%{ box-shadow:0 0 0 0 rgba(34,197,94,.4); } 70%{ box-shadow:0 0 0 8px rgba(34,197,94,0); } }
        ...
        .aR { animation: slideR .8s cubic-bezier(.16,1,.3,1) .3s both; }
        ...
        .sp  { animation: spin 14s linear infinite; }
        .mq  { animation: marquee 24s linear infinite; }
        .pulse-dot { animation: pulse2 2s infinite; }

        .lift { transition: transform .22s ease, box-shadow .22s ease; }
        .lift:hover { transform: translateY(-3px); box-shadow: 0 16px 40px rgba(0,0,0,.13); }
        .lift:active { transform: translateY(0); }

        .diagonal { clip-path: polygon(0 0, 100% 0, 100% 78%, 0 100%); }
        ...
        .card-hover:hover { transform: translateY(-6px); box-shadow: 0 24px 60px rgba(0,0,0,.12); }

        .cat-hover { transition: all .2s ease; }
        .cat-hover:hover { transform: scale(1.03); box-shadow: 0 12px 32px rgba(0,0,0,.1); }

        .text-gradient {
          background: linear-gradient(135deg, #000 30%, #666);
        ...
        .section-divider {
          width: 40px; height: 2px; background: #000; display: block; margin: 0;
        }
        ...
        /* Testimonial card */
        .testi-border { border: 1px solid #e5e5e5; }
        .testi-border:hover { border-color: #000; }
      `}</style>
